...
'''
todo::get the data to be uploaded to json file for one or 2 gas stations

'''


'''
http://www.hamiltongasprices.com/index.aspx?fuel=A&area=stoney%20creek&dl=Y&intro=Y
...
print("""
welcome!!
you will be asked which city/area in hamilton you want to look for gas in.
""")
...
if town == "hamilton" or town == "Hamilton":
    ...
    4: west
    """)
    area = int(input("enter area of hamilton: "))
    if area == 4:
        response = PlzOpen().open("http://www.hamiltongasprices.com/GasPriceSearch.aspx?area=Hamilton+-+West")
    elif area == 3:
        response = PlzOpen().open("http://www.hamiltongasprices.com/GasPriceSearch.aspx?area=Hamilton+-+East")
    elif area == 2:
        response = PlzOpen().open("http://www.hamiltongasprices.com/GasPriceSearch.aspx?area=Hamilton+-+South")
    elif area == 1:
        response = PlzOpen().open("http://www.hamiltongasprices.com/GasPriceSearch.aspx?area=Hamilton+mountain")

else:
    town=town.split()
    t = "%20".join(town)
    response = PlzOpen().open('http://www.'+city+'gasprices.com/index.aspx?fuel=A&area='+t+'&dl=Y&intro=Y')

#my_address = "315 Berkindale Drive, Hamilton ON"
...
for line in response:
    ...
    if "Gas_Stations" in str(line):
        ...
        find_brand = re.findall('<a href="/(.*?)_Gas_Stations', line_with_brand)
...
'''
I've noticed that getting the coords for my house sometimes leads to a noneType.
...
'''
also getting erros saying that noneType is not subscriptable -> throwing in
try & except to catch this error
'''
def calcDistance(geog_loca, lat, lng):
    ...
    for cord in range(len(geog_loca)):
        try:
            ...
            lng_to_gas = (lng - geog_loca[cord][1])*111.320*cos(lat-geog_loca[cord][0])
    ...
    if option == 1:
        ...
        for k in range(0,len(d)):
            if dist[k] <= 5.5:  #adding a 500m error
                print(str(gasprices[k]), "-->", brands[k], "at", locations[k], "which is about", dist[k], "km away")
            else:
                pass
        again()
